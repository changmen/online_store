...
public class ItemServiceImpl implements ItemService {
    ...
    public void addItem(String userId, Item item) {
        ...
        // 如果启用了OSS存储描述，则将描述内容上传到OSS
        if (useOssForDescription && StringUtils.isNotBlank(item.getDescription())) {
            String ossUrl = ossService.uploadItemDescription(item.getId(), item.getDescription());
            // 设置OSS URL
            item.setDescriptionUrl(ossUrl);
            // 清空原始描述内容，只保留URL引用
            item.setDescription("See description at: " + ossUrl);
        }
    ...
    public Item getItemById(long itemId) {
        ...
        if (cacheEnabled) {
            ...
            if (cachedItem != null) {
                ...
                if (useOssForDescription && StringUtils.isNotBlank(cachedItem.getDescriptionUrl())) {
                    loadItemDescriptionFromOss(cachedItem);
                }
        ...
        // 如果启用了OSS存储描述，则从OSS获取完整描述内容
        if (item != null && useOssForDescription && StringUtils.isNotBlank(item.getDescriptionUrl())) {
    ...
    public void updateItem(Item item) {
        ...
        if (useOssForDescription && StringUtils.isNotBlank(item.getDescription())) {
            // 检查描述是否已经是OSS URL
            if (!item.getDescription().startsWith("See description at:")) {
                String ossUrl = ossService.uploadItemDescription(item.getId(), item.getDescription());
     ...
     * @param item 商品对象
     */
    private void loadItemDescriptionFromOss(Item item) {
        if (StringUtils.isNotBlank(item.getDescriptionUrl())) {
            try {
                String fullDescription = ossService.getItemDescription(item.getDescriptionUrl());
                // 更新商品描述
                item.setDescription(fullDescription);
            } catch (Exception e) {
                logger.error("Failed to load item description from OSS: {}", item.getDescriptionUrl(), e);
                // 保留原始描述引用
            }
        }
    }
